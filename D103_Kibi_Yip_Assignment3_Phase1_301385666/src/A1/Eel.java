...
public class Eel extends Creatures{
	/*Theses are variables and methods are exclusive to this class. 
	  Some are methods from super class that are being override.
	  Methods that are exclusive to this class are traceBestFood(), attractedBy() and getAttraction().
		...
		for (int c=0; c<cList.size(); c++) {
			if (cList.get(c) instanceof Food) {
				...
				if (this.getBoundary().intersects(fish.getBoundary().getBounds2D())&&fish.getBoundary().intersects(this.getBoundary().getBounds2D())) {
					...
					cList.add(new Food((int)(Math.random()*screenSize.width),(int)(Math.random()*screenSize.height),Math.random()*0.5+0.5));
			}
		if (isTargetingFish){
