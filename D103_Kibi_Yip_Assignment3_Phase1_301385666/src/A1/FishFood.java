...
public class FishFood extends SimulatedObject {
	...
	public FishFood(Dimension screenSize) {
		...
		float x=(float) (Math.random()*(screenSize.width-size.width*scale)+size.width*scale/2);
		float y=(float) (Math.random()*(screenSize.height-size.width*scale)+size.width*scale/2);
		pos=new PVector(x,y);
