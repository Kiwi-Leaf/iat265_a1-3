...
//		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (int b=c+1;b<creatureList.size();b++) {
			if((creatureList.get(c) instanceof Eel || creatureList.get(c) instanceof Food)&&(creatureList.get(b) instanceof Eel || creatureList.get(b) instanceof Food)) {
			...
			if(creatureList.get(c).getClass()==creatureList.get(b).getClass()) {
					...
					if(fishTemp.collideWithEachOther(eelTemp)) {
						System.out.println("fish sees eel");
						fishTemp.reverseSmaller(eelTemp);
					}
					if(fishTemp.isEaten(eelTemp)) {
					creatureList.remove(fishTemp);
					creatureList.add(new Food((int)(Math.random()*screenSize.width),(int)(Math.random()*screenSize.height),Math.random()*0.2+0.5));
					}
